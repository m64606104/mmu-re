 ...
 * 获取联系人的记忆库
 */
export const getMemoryBank = (contactId: string): MemoryBank => {
...
export const addMemory = (
  ...
  const bank = getMemoryBank(contactId);
 ...
 * 删除记忆
 */
export const deleteMemory = (contactId: string, memoryId: string): void => {
  const bank = getMemoryBank(contactId);
 ...
 * 清空联系人的所有记忆
 */
export const clearMemoryBank = (contactId: string): void => {
  const bank = getMemoryBank(contactId);
  bank.memories = [];
 ...
 * 检查是否需要自动总结
 */
export const shouldTriggerAutoSummary = (contactId: string, currentMessageCount: number): boolean => {
  const bank = getMemoryBank(contactId);
 ...
 * 更新总结计数器
 */
export const updateSummaryCounter = (contactId: string, currentMessageCount: number): void => {
  const bank = getMemoryBank(contactId);
