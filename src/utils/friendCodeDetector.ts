...
1. **先回复信件的其他内容**（如果有的话）
2. **然后自然过渡到好友码话题**，在合适的地方**只**包含这个标记：\`[GIVE_PENPAL_CODE]\`
3. **重要：好友码是由系统生成和管理的固定值，你不要自己设计或修改号码**
   - ❌ 不要自己输出任何看起来像好友码的字符串，比如：\`PENPAL-XXXX\`、\`PENPAL-CUSTOM_XXXX\` 等
   - ✅ 只需要在合适的位置输出 \`[GIVE_PENPAL_CODE]\`，系统会自动把它替换成真实好友码
4. **示例**：
