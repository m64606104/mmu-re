...
【严格输出格式】
第一行：时间：HH:MM
第二行开始：直接写朋友圈文字（就像平时发朋友圈一样，不要加任何说明性文字）
${format.imageCount > 0 ? `后续${format.imageCount}行：每行一个[图片:简短2-3字描述]` : ''}

【示例】
时间：14:30
周末的午后，一杯咖啡一本书☕
${format.imageCount > 0 ? `[图片:咖啡]\n[图片:书本]` : ''}

⚠️ 绝对禁止：
❌ 不要写"朋友圈文字内容："、"文字："、"内容："等标签
❌ 不要写"图片数量：X张"、"图片描述"等说明
❌ 不要写长篇的画面描述，图片描述只需2-3个字
❌ 不要复述格式要求
...
【严格输出格式】
第一行：时间：${hour}:${minute.toString().padStart(2, '0')}
第二行：直接写朋友圈文字（像小孩子说话一样，不要加任何标签）
${imageCount > 0 ? `后续${imageCount}行：每行一个[图片:2-3字描述]` : ''}
