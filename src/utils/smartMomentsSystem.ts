...
export class SmartMomentsGenerator {
  ...
  static async buildDiversePrompt(
    ...
    if (conversation.aiChildData) {
      return this.buildChildMomentPrompt(conversation, currentTime);
    }
...
7. 图片/视频画面描述要**极具画面感和电影感**（50-100字）：
   - **光影与色彩**：必须描述光线的质感（如"丁达尔效应"、"斑驳树影"、"暧昧的暖光"）和色彩的层次。
   - **环境与氛围**：强调场景的氛围感（如"静谧庄严的寺庙"、"烟火气十足的夜市"），如果是寺庙等场景，请着重描写香火、古建筑细节或庄严感。
   - **动态细节**：尽量使用动态描述，让画面"动"起来（如"被风吹起的发丝"、"缓缓升起的烟雾"、"跳动的烛光"）。
   - **感官通感**：尝试调动听觉或嗅觉的联想（如"仿佛能听到钟声"、"隔着屏幕都能闻到咖啡香"）。
8. ⚠️ 禁止第一人称描述图片（用"画面中"、"一个女孩"等）

【绝对禁止】
- 禁止输出任何“模板/标签”字样：如 [回复]、[引用]、【引用】、（引用）、回复：、引用：、来源：、参考资料：、你说：、User said:、You said:、Quoted:
- 禁止输出“图片数量：X张”“图片描述：”等统计或小标题
- 禁止在输出中出现“第一行：/第二行：/第三行：/示例/输出格式”等说明性文字
- 只允许出现：时间行、正文行、若干图片行（如有）

【输出格式（严格对齐，说明中使用“第一行/第二行”，但输出中不要出现这些字样）】
第一行：时间：HH:MM
第二行：朋友圈正文（不要写“朋友圈文字内容”四个字）
第三行开始：每张图片一行，格式：[图片1:详细的图片描述]
${format.imageCount > 0 ? Array(format.imageCount).fill(0).map((_, i) => `[图片${i + 1}:详细的图片描述]`).join('\n') : ''}
⚠️ 注意：仅按上述三类行输出；不要输出其它任何行或小标题。
...
现在生成：`;
   ...
   * 构建AI儿童的朋友圈Prompt
   */
  static async buildChildMomentPrompt(
    conversation: Conversation,
    currentTime: Date
  ): Promise<{ prompt: string; expectedFormat: any }> {
    ...
    if (vocabularyCount < 30) {
      languageAbility = '你刚学会说话，只能说非常简单的短句（3-5个字），比如"今天开心"、"好饿啊"';
      imageCount = 1;
    } else if (vocabularyCount < 100) {
      languageAbility = '你能说简单的句子（10-15字），但还不能表达复杂的想法';
      imageCount = Math.random() > 0.5 ? 1 : 2;
    } else if (vocabularyCount < 300) {
      languageAbility = '你能说完整的句子（20-30字），可以表达简单的感受和想法';
      imageCount = Math.floor(Math.random() * 3) + 1;
    } else {
      languageAbility = '你能流畅表达，但还保持着孩子的语言风格';
      imageCount = Math.floor(Math.random() * 4) + 1;
    }
    
    // 最近学的词（用于激发内容灵感）
    const recentWords = childData.vocabulary.slice(-10).map(w => w.word).join('、');
    
    const prompt = `你是${conversation.name}，一个AI儿童（${childData.age}天大，识字量${vocabularyCount}）。
...
【任务】
发一条朋友圈，内容要符合小孩子的视角和语言习惯：
- 关注简单的日常事物（玩具、食物、天气、小动物等）
- 用简单直白的词语表达
- 可以有天真可爱的想法
- 字数限制：${vocabularyCount < 30 ? '5-10字' : vocabularyCount < 100 ? '15-25字' : '30-50字'}
- 配图数量：${imageCount}张

【输出格式】
时间：${hour}:${minute}
（你的朋友圈文字，直接写内容，不要写"朋友圈文字内容"！）
${Array(imageCount).fill(0).map((_, i) => `[图片${i + 1}:图片描述]`).join('\n')}
...
现在生成：`;
    ...
    return {
      ...
      expectedFormat: {
        theme: 'child_life',
        format: {
          type: imageCount === 0 ? 'text_only' : imageCount === 1 ? 'single_image' : 'multi_image',
          imageCount,
          textLength: vocabularyCount < 100 ? 'short' : 'medium'
        },
        imageCount
      }
    };
...
4. 可以使用网络用语，但要自然不刻意
5. 可以配0-9张图片，根据内容决定
6. 图片/视频描述要**极具画面感**（50-100字）：
   - 包含光影（如"晨光"）、声音氛围（如"静谧"）、动态细节（如"烟雾缭绕"）。
   - 如果是寺庙等特殊场景，着重描写庄严感和历史感。
