...
export async function buildTextChatRequest(
  ...
  const companionContextTail =
    ...
    '\n\n【引用回复（尾部机器指令，与换头像同类）】\n' +
    '- 下文每条历史消息前会带有〈消息ID:…〉；用户若在引用上文，会多一行〈引用目标:…〉。\n' +
    '- 当你本条气泡要对应某一条具体历史消息做引用展示时，在本条回复**最末尾**追加：[引用消息:消息ID]\n' +
    '- **消息ID**必须与该条前的〈消息ID:…〉**完全一致**（逐字符复制），禁止臆造；该标记会从气泡里剥掉，用户看不到。\n' +
    '- 不要用「回复我说的」之类话术当机器指令；不要复述〈…〉标签行。';
...
🎵 这是用户分享的网易云音乐，你可以讨论这首歌的旋律、歌词、歌手，或者分享你对这首歌的感受！`;
      ...
      return { role: m.role, content: `〈消息ID:${m.id}〉\n${content ?? ''}` };
    }),
  ];
